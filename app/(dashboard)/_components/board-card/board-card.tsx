...
export function BoardCard({
  ...
  const { mutate: favouriteMutation, pending: favouriteMutationPending } =
    useApiMutation(api.board.favourite);
  const { mutate: unFavouriteMutation, pending: unFavouriteMutationPending } =
    useApiMutation(api.board.unFavourite);

  const toggleFavourite = async () => {
    if (isFavourite) {
      try {
        await unFavouriteMutation({ id });
      } catch (error) {
        toast.error("Failed to un favourite");
      }
    } else {
      try {
        await favouriteMutation({ id, orgId });
      } catch (error) {
    ...
    <Link href={`/board/${id}`}>
      <div className="group aspect-100/127 border rounded-lg flex flex-col justify-between overflow-hidden">
        ...
        <Footer
          ...
          disabled={favouriteMutationPending || unFavouriteMutationPending}
        />
      </div>
    </Link>
  );
